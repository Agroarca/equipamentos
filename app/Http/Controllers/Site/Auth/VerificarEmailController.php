...
class VerificarEmailController extends Controller
{
    public function verificar(EmailVerificationRequest $request)
    {
        if ($request->user()->hasVerifiedEmail()) {
            return redirect()->route('site.perfil')->with('mensagem', 'Sua conta já foi verificada.');
        }
        $request->fulfill();

        if (!$request->hasValidSignature()) {
            return redirect()->route('site.perfil')->with('mensagem', 'Link de verificação inválido ou expirado');
        }
        return redirect()->route('site.perfil')->with('mensagem', 'Email verificado com sucesso');
    }
    ...
    public function reenviarEmail(Request $request)
    {
        ...
        return redirect()->route('site.perfil')->with('mensagem', 'Email de verificação será enviado em instantes.');
    }
}
