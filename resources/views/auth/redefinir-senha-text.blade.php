{{ config('app.name') }}

Olá
Você está recebendo este e-mail porque recebemos uma solicitação de redefinição de senha para sua conta.

Este link de redefinição de senha expirará em {{ $minutosExpiracao }} minutos.

Se você não solicitou uma redefinição de senha, nenhuma ação adicional será necessária.

Atenciosamente,
{{ config('app.name') }}

Se você está tendo problemas para clicar no botão "Redefinir senha", copie e cole este link em seu navegador da web:
{{ $url }}
